<template>
  <div class="approval" :class="approval.status">
    ...
    <div v-if="approval.status === 'pending'" class="actions">
      ...
      <n-button v-if="capabilities.can_approve" size="small" type="primary" @click="decide('approve')">仅批准本次</n-button>
      <n-button v-if="capabilities.can_approve && canGrant" size="small" secondary type="primary" @click="decide('approve', 'turn')">{{ grantLabel }}</n-button>
    </div>
...
const canGrant = computed(() => ['write_file', 'run_command'].includes(props.approval.tool_name))
const grantLabel = computed(() => props.approval.tool_name === 'write_file' ? '本轮允许写入' : '本轮允许相同命令')
...
const statusType = computed<'warning' | 'success' | 'error' | 'default'>(() => {
  if (props.approval.status === 'pending') return 'warning'
  if (props.approval.status === 'approved') return 'success'
  if (props.approval.status === 'rejected') return 'error'
  return 'default'
})
async function decide(decision: 'approve' | 'reject', scope: 'once' | 'turn' = 'once') {
...
<style scoped>
.approval { margin: 0 0 14px; padding: 13px 14px; border: 1px solid var(--border-strong); border-radius: 14px; background: var(--bg-soft); }
.approval.pending { border-color: color-mix(in srgb, var(--accent) 42%, var(--border-strong)); }
.head { display: flex; align-items: center; justify-content: space-between; gap: 10px; }
.head > div { display: flex; align-items: baseline; gap: 8px; }
.eyebrow { font-size: 11px; color: var(--text-3); text-transform: uppercase; letter-spacing: .08em; }
.risk { margin-top: 8px; font-size: 13px; color: var(--text-2); }
.args { margin: 9px 0 0; padding: 9px 10px; max-height: 220px; overflow: auto; border-radius: 9px; background: var(--bg); border: 1px solid var(--border); font-size: 12px; white-space: pre-wrap; word-break: break-word; }
.actions { display: flex; justify-content: flex-end; gap: 7px; margin-top: 10px; flex-wrap: wrap; }
.challenge { display: flex; gap: 7px; margin-top: 10px; }
</style>
