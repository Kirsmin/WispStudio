<template>
  <div class="approval" :class="approval.status">
    ...
    <div v-if="approval.status === 'pending'" class="actions">
      ...
      <n-button v-if="capabilities.can_approve" size="small" type="primary" @click="decide('approve')">批准原操作</n-button>
    </div>
...
async function decide(decision: 'approve' | 'reject') {
  try { await chatStore.decideApproval(props.approval.id, decision) } catch (error) { window.$message?.error(error instanceof Error ? error.message : String(error)) }
}
